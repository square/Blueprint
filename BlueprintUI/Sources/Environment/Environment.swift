...
extension Environment: ContextuallyEquivalent {
    ...
    func isEquivalent(to other: FrozenEnvironment?, in context: EquivalencyContext) -> Bool {
        ...
        if frozen.fingerprint == fingerprint { return true }
...
extension CacheStorage {
    ...
    fileprivate struct FingerprintCache {

        typealias EquivalencyResult = [EquivalencyContext: Bool]
        var storage: [ComparableFingerprint: [EquivalencyContext: Bool]] = [:]

        public subscript(fingerprint: ComparableFingerprint, context: EquivalencyContext) -> Bool? {
            get {
                if let exact = storage[fingerprint]?[context] {
                    return exact
                } else if let allComparisons = storage[fingerprint] {
                    switch context {
                    case .all:
            ...
            set {
                storage[fingerprint, default: [:]][context] = newValue
            }
        }
    ...
    private struct EnvironmentComparisonCacheKey: CacheKey {
        static var emptyValue = FingerprintCache()
    }

    fileprivate var environmentComparisonCache: FingerprintCache {
