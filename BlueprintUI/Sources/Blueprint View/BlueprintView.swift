...
public final class BlueprintView: UIView {
    ...
    public required init(element: Element?, frame : CGRect = .zero, animateInitialLayout : Bool = UIView.isInAnimationBlock) {
        ...
        rootController = NativeViewController(
            ...
            parentContext: UpdateContext(
                ...
                hasParentAppearanceTransition: false
            ),
            update: true // TODO: Should be false? seems like probably yes? Also, just remove update from here entirely?
        )
    ...
    public override func didMoveToWindow() {
        ...
        switch self.neededUpdateType {
        case .none: self.neededUpdateType = .needsViewHierarchyUpdate(animated: false)
        case .needsViewHierarchyUpdate(let animated): self.neededUpdateType = .needsViewHierarchyUpdate(animated: animated)
        }
    }
...
extension BlueprintView {
    ...
    struct UpdateContext {
        ...
        init(
            ...
            hasParentAppearanceTransition : Bool
        ) {
            self.allowsAnimations = allowsAnimations
            self.hasParentAppearanceTransition = hasParentAppearanceTransition
        }
        
        // MARK: Animations Enabled
        
        private(set) var allowsAnimations : Bool
        ...
        private var hasParentAppearanceTransition : Bool
