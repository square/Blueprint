...
public final class BlueprintView: UIView {
    ...
    /// Flag used to turn on cross layout caching for all blueprint views
    /// This overrides the local flag set on each view
    public static var enableCrossLayoutCachingGlobally: Bool = false

    /// Flag used to turn on cross layout caching on a single view
    public var enableCrossLayoutCaching: Bool = false
    private var isCrossLayoutCachingEnabled: Bool { Self.enableCrossLayoutCachingGlobally || enableCrossLayoutCaching }
    ...
    /// Updates the layout caching flag for `ElementState` only from root level `BlueprintView`s
    /// For out-of-band renders, this should re-use the flag value used by the parent view.
    /// This assumes all blueprint layout calls happen on the main thread.
    private func updateLayoutCachingFlag(_ closure: () -> Void) {
        print("cross layout caching: \(ElementState.enableCrossLayoutCaching)")
        var shouldClearCrossLayoutCaching = false
        if ElementState.enableCrossLayoutCaching == nil {
            shouldClearCrossLayoutCaching = true
            ElementState.enableCrossLayoutCaching = isCrossLayoutCachingEnabled
        }

        defer {
            if shouldClearCrossLayoutCaching {
                ElementState.enableCrossLayoutCaching = nil
            }
        }
        closure()
    }
