...
extension StackElement {
    ...
    public mutating func addFlexibleSpacer() {
        precondition(
            ...
            """
            Flexible spacers have no effect if the Stack's underflow does not stretch its contents. \
    ...
    /// Adds a fixed `Spacer` element to the stack, with the provided size – width for rows, and height for columns.
    ///
    /// ### Note
