...
public struct GridRow: Element {
    ...
    // MARK: - mutations -
    public mutating func add(child: Child) {
...
public extension GridRow {
    ...
    struct Child {
        ...
        public static func absolute(_ width: CGFloat, _ element: Element) -> Child {
            Child(width: .absolute(width), element: element)
        }

        public static func proportional(_ width: CGFloat, _ element: Element) -> Child {
            Child(width: .proportional(width), element: element)
        }
    }
...
extension GridRow {
    struct GridRowLayout: Layout {
        ...
        /// proportionally-sized children. When layout width is known,
        ///
        ...
        private func unconstrainedProportionalItemSizes(
            ...
            return items.map { (index, proportion, content) in
                let width = scale * proportion
                let constraint = SizeConstraint(width: .atMost(width), height: constraint.height)
                let size = CGSize(width: width, height: content.measure(in: constraint).height)
                return (index, size)
            }
        }
